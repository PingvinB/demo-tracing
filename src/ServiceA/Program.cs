...
builder.Services.AddOpenTelemetry()
    .ConfigureResource(r => r.AddService(serviceName: "service-a"))
    .WithTracing(c =>
    {
        ...
        c.AddSource("service-a")
            .AddAspNetCoreInstrumentation(cfg =>
            {
                cfg.Filter = context =>
                {
                    ...
                    var shouldInclude = !excludePrefixes.Any(prefix => context.Request.Path.StartsWithSegments(prefix));
